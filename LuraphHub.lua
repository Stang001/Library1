...
for _,v in pairs(game.CoreGui:GetChildren()) do
    if v.Name == "LuraphHub" then
        v:Destroy()
    end
end

local LuraphHub = Instance.new("ScreenGui")
LuraphHub.Name = "LuraphHub"
LuraphHub.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
LuraphHub.Parent = game.CoreGui
...
function Library:Create(table)
    ...
    shadow.ZIndex = -1
    shadow.Parent = LuraphHub

    tabContainer.Parent = main
    main.Parent = LuraphHub
    ...
    function tabHandler:Tab(name)
        ...
        main1.MouseButton1Click:Connect(function()
            for _,v in pairs(game.CoreGui:FindFirstChild('LuraphHub').main:GetChildren()) do
                if v.Name == "container" then
                    v.Visible = false
                end
            end
            for _,v in pairs(game.CoreGui['LuraphHub'].main.tabContainer:GetChildren()) do
        ...
        local Input = main.InputBegan:connect(function(Key)
            if Key.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                while RunService.RenderStepped:wait() and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
                    local FrameX, FrameY = math.clamp(Mouse.X - ObjectPosition.X, 0, LuraphHub.AbsoluteSize.X - main.AbsoluteSize.X), math.clamp(Mouse.Y - ObjectPosition.Y, 0, LuraphHub.AbsoluteSize.Y - main.AbsoluteSize.Y)
    ...
    game.CoreGui['LuraphHub'].main.tabContainer.ChildAdded:Connect(function()
        game.CoreGui['LuraphHub'].main:FindFirstChild('container').Visible = true
        pcall(function()
            repeat wait() until game.CoreGui['LuraphHub'].main:FindFirstChild('tabContainer'):FindFirstChildWhichIsA('TextButton')
        end)
        game:GetService('TweenService'):Create(game.CoreGui['LuraphHub'].main:FindFirstChild('tabContainer'):FindFirstChildWhichIsA('TextButton'), TweenInfo.new(0.3), {TextTransparency = 0}):Play()
    end)
